...
from plotting import SimglucoseProgressPlotCallback, RewardCurveCallback


ENV_CHOICES = ("env_closed", "env_open", "env_closed_action_history")


def load_env_module(env_name: str):
    """Load an environment module from the local envs package."""
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="env_closed",
        choices=ENV_CHOICES,
        help="Environment module to use from the envs/ package.",
    )
    ...
    parser.add_argument(
        ...
        default="default",
        help=(
            "Reward type. Valid values depend on the selected --env module "
            "and are checked after the environment is loaded."
        ),
    )
...
def main() -> None:
    ...
    reward_curve_callback = RewardCurveCallback(
        save_dir=outdir / "progress",
        plot_freq=10_000,
        rolling_window=25,
        verbose=1,
    ...
    # Trick: Add learning rate decay
    def linear_schedule_with_floor(initial_lr: float, final_fraction: float = 0.1):
        def schedule(progress_remaining: float):
            return initial_lr * (
                final_fraction + (1 - final_fraction) * progress_remaining
            )
        return schedule
    ...
    model = PPO(
        policy="MlpPolicy",
        env=train_env,
        learning_rate=linear_schedule_with_floor(config.learning_rate),
