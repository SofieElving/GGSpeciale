...
# (minute_of_day, grams_carbohydrate)
DEFAULT_MEALS = [
    (7 * 60, 45.0),
    (12 * 60, 70.0),
    (16 * 60, 15.0),
...
class SimglucoseFeatureWrapper(gym.Wrapper):
    ...
    All meal times are represented in minutes after midnight.

    Important:
    - The action is clipped before being passed to SimGlucose.
    - raw_policy_action is still logged, so the policy can be penalized
      for trying to give excessive insulin.
    """
    ...
    def __init__(
        ...
        self.use_custom_reward = bool(use_custom_reward)
        ...
        # Keep native SimGlucose action space.
    ...
    def reset(self, **kwargs):
        ...
        self._add_plot_info(info, obs, insulin=0.0, raw_action=0.0)
    ...
    def step(self, action):
        ...
        if self.use_custom_reward:
            reward = glucose_insulin_reward(
                bg=raw_cgm,
                insulin=delivered,
                raw_insulin=raw_action,
                max_insulin_action=self.max_insulin_action,
            )

        info["original_reward"] = original_reward
        self._add_plot_info(info, obs, insulin=delivered, raw_action=raw_action)
    ...
    def _extract_minute_of_day(self, info: dict) -> float:
        ...
        # Fallback if SimGlucose does not provide time.
    ...
    def _next_meal_features(self) -> tuple[float, float]:
        ...
        Returns:
            meal_warning:
                Smooth warning signal in [0, 1].
                Larger value means meal is closer.

            meal_size:
                Carbohydrates of the next scheduled meal.
        """
        if not self.meal_schedule:
            return 0.0, 0.0
...
def build_scenario_and_wrapper_schedule(
    ...
    Builds:
        sim_scenario:
            Scenario passed to SimGlucose.

        wrapper_schedule:
            Same meals in minute format for feature construction.
    """
...
def register_simglucose_gym_env(
    ...
    """
    Register SimGlucose as a Gymnasium environment.

    The registry is global, so existing IDs are removed first.
    """
...
def make_simglucose_spid_env(
    ...
    """
    Create one wrapped SimGlucose environment.
    """
...
class MultiPatientSimglucoseEnv(gym.Env):
    """
    Samples one patient at each reset.

    This is useful for training one policy across multiple virtual adults.
    """
    ...
    def __init__(
        ...
        # Probe first patient to expose observation/action spaces.
        probe_env = make_simglucose_spid_env(
            ...
            use_custom_reward=self.use_custom_reward,
            max_insulin_action=self.max_insulin_action,
        )
