...
"""
Stable-Baselines3 callbacks for saving training diagnostics.
"""
...
class SimglucoseProgressPlotCallback(BaseCallback):
    """Evaluate the current policy and save glucose, insulin, and IOB plots."""
    ...
    def _run_rollout_and_save(self) -> None:
        ...
        metrics = self._compute_metrics(
            ...
            insulin_actions=insulin_actions,
            proposed_insulins=proposed_insulins,
            shielded_insulins=shielded_insulins,
        ...
        if self.verbose > 0:
            print(f"Saved progress plot in: {self.save_dir}")
            print(f"Updated metrics file: {self.metrics_path}")
    ...
    def _append_metrics_csv(self, metrics: dict[str, float | int]) -> None:
    ...
    def _save_metrics_json(self, metrics: dict[str, float | int]) -> None:
    ...
    def _save_progress_plot(
        ...
        shield_times = [t for t, active in zip(times, shield_active) if active > 0.5]
        shield_cgm_values = [c for c, active in zip(cgms, shield_active) if active > 0.5]
        ...
        axes[1].plot(times, meals, label="Meals")
        axes[1].set_ylabel("Meals")
        axes[1].legend(loc="upper right")

        axes[2].plot(times, shielded_insulins, label="Delivered insulin")
        shield_insulin_values = [
            y for y, active in zip(shielded_insulins, shield_active) if active > 0.5
        ]
...
class RewardCurveCallback(BaseCallback):
    """
    Track episode rewards during training and repeatedly save:
    - reward_curve.png
    - reward_curve.csv
    """
    ...
    def _on_step(self) -> bool:
        ...
        if self.episode_rewards and self.num_timesteps % self.plot_freq == 0:
            self._save_csv()
            self._save_plot()
    ...
    def _rolling_mean(self, values: list[float]) -> np.ndarray | None:
        values_arr = np.asarray(values, dtype=float)
        if len(values_arr) < self.rolling_window:
            return None
    ...
    def _save_csv(self) -> None:
        with self.csv_path.open("w", newline="", encoding="utf-8") as f:
            ...
            for i, (timestep, reward, length) in enumerate(
                zip(
                ...
                writer.writerow([i, timestep, reward, length])
    ...
    def _save_plot(self) -> None:
        ...
        rolling = self._rolling_mean(self.episode_rewards)
