...
def get_patient_bw_and_kind(patient_name: str) -> tuple[float, str]:
    vpatient_params = pd.read_csv(PATIENT_PARA_FILE)
    bw = vpatient_params.query("Name == @patient_name")["BW"].item()
    kind = patient_name.split("#")[0]
    return float(bw), kind


def harris_benedict(weight: float, kind: str) -> tuple[float, float, float, float]:
    """
    ...
    if kind == "child":
        child_weight_to_age_and_height = [
            ...
            (32.0, 35.6, 10, 138.4),
            (35.6, 39.9, 11, 143.5),
            (39.9, np.inf, 12, 149.1),
        ]
        for lo, hi, age, height in child_weight_to_age_and_height:
            if lo < weight <= hi:
                break
        ...
        adolescent_weight_to_age_and_height = [
            ...
            (66.9, 68.9, 18, 175.7),
            (68.9, 70.3, 19, 176.5),
            (70.3, np.inf, 20, 177.0),
        ]
        for lo, hi, age, height in adolescent_weight_to_age_and_height:
            if lo < weight <= hi:
                break
...
class SemiRandomHarrisonBenedictScenario(Scenario):
    ...
    def __init__(
        ...
        self.bw, self.kind = get_patient_bw_and_kind(patient_name)
        self.reset()
