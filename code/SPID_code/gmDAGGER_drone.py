...
from PySRWrapper_fid_per_gap import PySRPolicy
...
def save_final_results_json(path, teacher_metrics, student_metrics, best_iteration, dataset_size, fidelity_losses, perf_gaps, gm_losses, fidelity_losses_eval, perf_gaps_eval, gm_losses_eval):
    payload = {
        ...
        "loss_components": [
            {
                "iteration":      int(i),
                "mean_fidelity":  float(f),
                "mean_perf_gap":  float(p),
                "mean_gm_loss":   float(g)
            }
            for i, (f, p, g) in enumerate(zip(fidelity_losses, perf_gaps, gm_losses))
        ],
        "loss_components_eval": [
            {
                "iteration":      int(i),
                "mean_fidelity":  float(f),
                "mean_perf_gap":  float(p),
            ...
            for i, (f, p, g) in enumerate(zip(fidelity_losses_eval, perf_gaps_eval, gm_losses_eval))
        ]
    }
...
def train_spid(
    ...
    for i in tqdm(range(n_iter), disable=verbose > 0):
        ...
        loss_components = new_data[-1]
        new_data = new_data[:-1]  # keep dataset handling unchanged

        fidelity_losses.append(loss_components["mean_fidelity"])
        perf_gaps.append(loss_components["mean_perf_gap"])
        gm_losses.append(
            float(np.sqrt(loss_components["mean_fidelity"] * loss_components["mean_perf_gap"]))
        )
        ...
        expression_losses = compute_expression_losses(srr_test, x, y, weights)
        fidelity_losses_eval.append(expression_losses["mean_fidelity"])
        perf_gaps_eval.append(expression_losses["mean_perf_gap"])
        gm_losses_eval.append(expression_losses["gm_loss"])

        add_fidelity_losses_to_equations(srr_test, x, y, weights)
        ...
        if verbose == 2:
            print(
                f"Iteration {i}: student reward = {mean_reward:.4f} +/- {std_reward:.4f}"
                f"fidelity = {loss_components['mean_fidelity']:.4f} +/- {loss_components['std_fidelity']:.4f}"
                f"perf_gap = {loss_components['mean_perf_gap']:.4f} +/- {loss_components['std_perf_gap']:.4f}"
                f"expression_fidelity = {expression_losses['mean_fidelity']:.4f}"
                f"expression_perf_gap = {expression_losses['mean_perf_gap']:.4f}"
                f"expression_gm_loss = {expression_losses['gm_loss']:.4f}"
                )
    ...
    if save_results:
        ...
        save_loss_components_csv(
            gm_losses, fidelity_losses, perf_gaps, run_dir / "loss_components.csv"
        )
        save_loss_components_csv(
            gm_losses_eval, fidelity_losses_eval, perf_gaps_eval, run_dir / "loss_components_eval.csv"
        )
    ...
    if save_results:
        save_final_results_json(
            ...
            perf_gaps=perf_gaps,
            gm_losses=gm_losses,
            fidelity_losses_eval=fidelity_losses_eval,
            perf_gaps_eval=perf_gaps_eval,
            gm_losses_eval=gm_losses_eval,
        )
...
def sample_trajectory(
    ...
    valid = (
        ...
        np.isfinite(teacher_actions).all(axis=1) & 
        np.isfinite(weights)
    )

    states = states[valid]
    teacher_actions = teacher_actions[valid]
    training_actions = training_actions[valid]
    weights = weights[valid]

    fidelity_per_step = np.mean(
        (training_actions - teacher_actions) ** 2, axis=-1
    )

    loss_components = {
        "mean_fidelity": float(np.mean(fidelity_per_step)),
        "std_fidelity":  float(np.std(fidelity_per_step)),
        "mean_perf_gap": float(np.mean(np.abs(weights))),
        "std_perf_gap":  float(np.std(np.abs(weights))),
    }

    trajectory = [states, teacher_actions, weights, loss_components]
...
def save_loss_components_csv(gm_losses, fidelity_losses, perf_gaps, path):
    path = Path(path)
    with path.open("w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["iteration", "mean_gm_loss", "mean_fidelity", "mean_perf_gap"])
        for i, (g, f, p) in enumerate(zip(gm_losses, fidelity_losses, perf_gaps)):
            writer.writerow([i, g, f, p])



def compute_expression_losses(policy, x, teacher_actions, weights, epsilon=1e-6, alpha=1):
    """
    Compute loss components by evaluating the symbolic expression
    directly on the collected states.
    """
    # pred = np.array([policy.predict(obs.reshape(1, -1))[0] for obs in x]).reshape(teacher_actions.shape)
    preds = []
    for obs in x:
        action, _ = policy.predict(obs.reshape(1, -1))
        preds.append(np.array(action).reshape(-1))
    
    pred = np.array(preds).reshape(teacher_actions.shape)

    fidelity_per_step = np.mean((pred - teacher_actions) ** 2, axis=-1)
    
    fidelity  = float(np.mean(fidelity_per_step))
    perf_gap  = float(np.mean(weights * fidelity_per_step + alpha))
    gm_loss   = float(np.sqrt(fidelity * perf_gap))

    return {
        "mean_fidelity": fidelity,
        "mean_perf_gap": perf_gap,
        "gm_loss":       gm_loss,
    }


def add_fidelity_losses_to_equations(policy, x, teacher_actions, weights, alpha=1.0, epsilon=1e-6):
    """
    Evaluates every expression in each PySRWrapper's equations_ dataframe
    and adds fidelity, performance, and GM loss as new columns.
    """
    for output_idx, pysr_wrapper in enumerate(policy.policy_list):
        sr = pysr_wrapper.sr
        eq_df = sr.equations_

        # Teacher actions for this output dimension
        t_act = teacher_actions[:, output_idx] if teacher_actions.ndim > 1 else teacher_actions

        fidelity_losses = []
        perf_gaps       = []
        gm_losses       = []

        for expr_idx in range(len(eq_df)):
            pred = sr.predict(x, index=expr_idx)
            pred = np.array(pred).reshape(t_act.shape)

            fidelity_per_step = (pred - t_act) ** 2
            fidelity = float(np.mean(fidelity_per_step))
            perf_gap = float(np.mean(weights * fidelity_per_step + alpha))
            gm_loss  = float(np.sqrt(fidelity * perf_gap))

            fidelity_losses.append(fidelity)
            perf_gaps.append(perf_gap)
            gm_losses.append(gm_loss)

        eq_df["fidelity_loss"] = fidelity_losses
        eq_df["perf_gap"]      = perf_gaps
        eq_df["gm_loss"]       = gm_losses
